...
class Brains
	...
	private:
		...
		uint16_t myHysteresiscentiTemp;  //the desired +- hysteresis temperature difference
		TempMeterInterface& myTempSensor;  //the sensor
		uint32_t myTotalOnTime;//the total time the relais was on

public:
		Brains (uint8_t relaisPin,uint16_t desiredcentiTemp,uint16_t hysteresiscentiTemp,TempMeterInterface &tempSensor);
		void setup();
		void loop();
		boolean isFridgeOn();
		uint32_t getOnTime();
